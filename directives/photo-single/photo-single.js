		...
		return {
			...
			controller: function($scope){
				$scope.comment = function(text){
					photoService.comment($scope.photoId,{text:text}).then(function(response){
						var date = new Date()
						$scope.photo.actions.comments.push({picture:$scope.user.profileImg,displayName:$scope.user.displayName,text:text,dt_create:date})
						$scope.yourcomment = '';
					},function(err){
						console.log(err)
					})
				...
				$scope.vote = function(operation){
					photoService.vote($scope.photoId, {type:operation} ).then(function(response){
						if(operation=='upvote')
							$scope.voted = false;
						else
							$scope.voted = true;
				...
				$scope.rate = function(rating){
					if(!$scope.isReadonly)
						photoService.rate($scope.photoId,{rate:rating}).then(function(response){
